...
```
here here borrowedAsset = borrowed token in ChainB.
...
```
here if we observe payload.srcToken = borrow token of chain B.
...
```
here payload.srcToken = chainB Borrow token.
...
```
here it is giving input (srcToken =  payload.srcToken) where payload.srcToken = chainB borrow token. But here we should give borrowed token of chainA corresponding to borrowed token in this chain as input srcToken(because this is what stored in crossChainCollaterals[user][underlying] struct).Due to this  lendStorage.findCrossChainCollateral will return bool found = false.
...
using payload.srcToken instead of using chain A borrowed then in below lines of code,
https://github.com/sherlock-audit/2025-05-lend-audit-contest/blob/713372a1ccd8090ead836ca6b1acf92e97de4679/Lend-V2/src/LayerZero/CrossChainRouter.sol#L454
```solidity
            payload.srcToken
```
...
use  chain A borrowed token instead of using payload.srcToken  in below lines of code,
https://github.com/sherlock-audit/2025-05-lend-audit-contest/blob/713372a1ccd8090ead836ca6b1acf92e97de4679/Lend-V2/src/LayerZero/CrossChainRouter.sol#L454
```solidity
            payload.srcToken
```