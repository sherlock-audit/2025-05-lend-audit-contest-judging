...
```
here underlying address should be address of the borrowed token in chain B, but instead here value of underlying = address(0) as we are using payload.destltoken = chainA Ltoken of collateral as input in  ltokenTounderlying function, so that function will  return address(0).
and  lendStorage.findCrossChainCollateral will return bool found = false.
...
using (payload.destlToken = chainA collateral Ltoken) in place of Ltoken of borrowed token in chain B in below lines of code ,
